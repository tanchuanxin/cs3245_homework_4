...
FOLDERSIZE_LIMIT=800
echo -e "Checking if unzipped size of $zipfile is SMALLER THAN ${FOLDERSIZE_LIMIT}MB..."
folderSize=$(( $(unzip -l $zipfile | tail -1 | xargs | cut -d' ' -f1)/1000/1000 ))
if [ "$folderSize" -ge "$FOLDERSIZE_LIMIT" ]; then
		echo -e "\tFAILED: Folder size is ${folderSize}MB. Exiting..."
		exit 1
else
		echo -e "\tPASSED: Folder size is ${folderSize}MB"
fi
